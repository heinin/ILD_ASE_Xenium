...
#merged_spatial <- readRDS("/scratch/hnatri/ILD/ILD_spatial_ASE/merged_spatial_filtered.rds")
#merged_spatial[["base_RNA"]] <- NULL

# Adding an assay with only base panel, no allele specific probes. This assay
# will be used for dimensionality reduction and clustering.
features <- rownames(merged_spatial)
features <- features[-grep("-", features)]

merged_spatial_base <- subset(merged_spatial, features = features)

tmp1 <- LayerData(merged_spatial_base, assay = "RNA")
tmp2 <- LayerData(merged_spatial, assay = "RNA")
...
merged_spatial_base[["RNA"]]
merged_spatial_base[["cell_RNA"]] <- NULL

# Saving the object for clustering with ScanPy
#saveRDS(merged_spatial_base, "/scratch/hnatri/ILD/ILD_spatial_ASE/merged_spatial_filtered_base.rds")

# To build on command line, run Rscript -e "rmarkdown::render('Xenium_preprocessing.Rmd')"
